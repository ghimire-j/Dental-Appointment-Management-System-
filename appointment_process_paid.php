...
if ($check_result) {
    ...
    if ($check_row['count'] <= 0) {
        ...
        $query = "INSERT INTO appointment_form (first_name, last_name, email, phone, Preferred_doctor, appointment_date, Symptoms, status) VALUES ('$first_name', '$last_name', '$email', '$phone','$preferred_doctor', '$appointment_date', '$Symptoms', 'Booked (100)')";
        if (mysqli_query($conn, $query)) {
            ...
            $message = "Payment has been done for the appointment on $appointment_date.";
...
if (isset($_POST['cancel'])) {
    ...
    if (mysqli_query($conn, $update_query)) {
        // Retrieve appointment details for notification
        $cancel_query = "SELECT * FROM appointment_form WHERE appointment_id='$appointment_id'";
        ...
        // Show a success message and prompt for booking a new appointment
        echo "<script>
...
if ($reminder_result) {
    while ($reminder_row = mysqli_fetch_assoc($reminder_result)) {
        ...
        $subject = "Reminder: Your appointment  with" . $reminder_row['Preferred_Doctor'];
...
<body>
    <div class='content'>
        ...
        <div class='zz'>
            ...
            <form method='post' action='cancel.php' style="text-align:center">
